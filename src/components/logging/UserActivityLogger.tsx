...
export default function UserActivityLogger() {
  ...
  const formatDate = (dateString: string) => {
    return new Date(dateString).toLocaleString('es-ES', { timeZone: 'America/Bogota' })
  }
      ...
      <CardContent>
        <div className="space-y-4">
          <div className="border rounded-md p-4 max-h-96 overflow-y-auto">
            <h3 className="font-semibold mb-2">Sesiones de Usuario:</h3>
            <Table>
              <TableHeader>
                <TableRow>
              ...
              <TableBody>
                {sessions.map((session, index) => (
                  <TableRow key={index}>
                    ...
                    <TableCell>{formatDate(session.sessionStart)}</TableCell>
                    <TableCell>{session.sessionEnd ? formatDate(session.sessionEnd) : 'Activa'}</TableCell>
                    <TableCell>
                      {session.actions.map((action, actionIndex) => (
                        <div key={actionIndex} className="mb-2">
                          <div className="font-semibold">{formatDate(action.timestamp)}</div>
                          <div>{action.actionType}</div>
                          {action.contractId && <div>Contrato: {action.contractId}</div>}
                          {action.documentId && <div>Documento: {action.documentId}</div>}
                        </div>
                      ))}
                    </TableCell>
                  </TableRow>
